# Se importan las bibliotecas para mandar los request al server
...
def desafio1(mensaje):
    # Primero ciframos el mensaje con el rot 8
    mensajeCifrado = rotN(mensaje, 8)
    # Luego por vignere con la contraseña heropassword
    mensajeCifrado = vigenere(mensajeCifrado, "heropassword")
    # Luego por rot 12
    mensajeCifrado = rotN(mensajeCifrado, 12)

    # Esto que es para mandar el resultado al server
    headers = {"Content-Type": "text/plain"}
    data = '{"msg":' + mensajeCifrado + "}"
    response = requests.post(
        "https://finis.mmae.cl/SendMsg", headers=headers, data=data
    )
    return mensajeCifrado


# Función para el desafío 2
def desafio2(custom, mensajeCustom):
    # Este es el if para que descifremos un mensaje personalizado
    if custom:
        # primero desciframos el mensaje por rot -12
        mensajeDescifrado = rotN(mensajeCustom, -12)
        # Luego por vignere
        mensajeDescifrado = des_vigenere(mensajeDescifrado, "finispasswd")
        # luego por rot -8
        mensajeDescifrado = rotN(mensajeDescifrado, -8)
        return mensajeDescifrado
    # Este es el desafío 2 usando el string del server
    else:
        #Acá sacamos el string del server
        headers = {
            "Content-Type": "text/plain",
        }
        response = requests.get("https://finis.mmae.cl/GetMsg", headers=headers)
        mensaje_dict = json.loads(response.text)
        mensaje = mensaje_dict["msg"]
        # primero desciframos el mensaje por rot -12
        mensajeDescifrado = rotN(mensaje, -12)
        # Luego por vignere
        mensajeDescifrado = des_vigenere(mensajeDescifrado, "finispasswd")
        # luego por rot -8
        mensajeDescifrado = rotN(mensajeDescifrado, -8)
